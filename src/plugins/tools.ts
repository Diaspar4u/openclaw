...
export function resolvePluginTools(params: {
  ...
  // In a multi-workspace process the loader's own cache-key invalidation
  // handles workspace changes, so falling through to loadOpenClawPlugins
  // when no active registry exists is the correct path for that case.
  ...
  const activeRegistry = getActivePluginRegistryVersion() > 0 ? getActivePluginRegistry() : null;
  const registry =
    activeRegistry ??
    loadOpenClawPlugins({
