...
import { resolveMutationGateConfig } from "./pi-tools.js";
...
describe("checkMutationGate deadlock prevention", () => {
  it("skips gate when hasInlineButtons is false", () => {
    const result = checkMutationGate({
      toolName: "gateway",
      params: {},
      sessionKey: "agent:dev:telegram:group:-100123:topic:42",
  ...
  it("skips gate when hasMessageTool is false", () => {
    const result = checkMutationGate({
      toolName: "gateway",
      params: {},
      sessionKey: "agent:dev:telegram:group:-100123:topic:42",
  ...
  it("enforces gate when both hasInlineButtons and hasMessageTool are true", () => {
    const result = checkMutationGate({
      toolName: "gateway",
      params: {},
      sessionKey: "agent:dev:telegram:group:-100123:topic:42",
  ...
  it("enforces gate when hasInlineButtons and hasMessageTool are undefined (default)", () => {
    const result = checkMutationGate({
      toolName: "gateway",
      params: {},
      sessionKey: "agent:dev:telegram:group:-100123:topic:42",
...
describe("isMemoryFileWrite", () => {
  ...
  it("rejects deeply nested memory dir (not workspace root level)", () => {
    expect(
      isMemoryFileWrite("write", { file_path: `${workspace}/docs/memory/changelog.md` }, workspace),
    ).toBe(false);
  });

  it("rejects memory subdir nested under other dirs", () => {
    expect(
      isMemoryFileWrite(
        "write",
        { file_path: `${workspace}/some/deep/memory/notes.md` },
        workspace,
      ),
    ).toBe(false);
  });
...
describe("DEFAULT_MUTATION_TOOLS immutability", () => {
  it("is a readonly array that cannot be mutated via push", () => {
    // TypeScript enforces readonly at compile time; at runtime verify the
    // exported reference is an array and its content stays stable.
    const before = [...DEFAULT_MUTATION_TOOLS];
    // Attempting (DEFAULT_MUTATION_TOOLS as string[]).push("evil") would
    // corrupt shared state.  Verify the array is not accidentally writable
    // by checking that the snapshot matches after a gate check cycle.
    checkMutationGate({
      toolName: "gateway",
      params: {},
      sessionKey: "agent:dev:telegram:group:-100123:topic:42",
    ...
    expect([...DEFAULT_MUTATION_TOOLS]).toEqual(before);
  });
});

describe("approval TTL eviction", () => {
  it("evicts stale entries when a new approval is recorded", () => {
    // Seed a stale entry by back-dating its timestamp
    MUTATION_APPROVALS.set("stale-session", {
      timestamp: Date.now() - APPROVAL_STALENESS_TTL_MS - 1,
      senderId: "old-user",
    });
    expect(MUTATION_APPROVALS.has("stale-session")).toBe(true);

    // Recording a new approval triggers eviction
    recordMutationApproval("fresh-session", "new-user");

    expect(MUTATION_APPROVALS.has("stale-session")).toBe(false);
    expect(MUTATION_APPROVALS.has("fresh-session")).toBe(true);
  });

  it("retains non-stale entries during eviction", () => {
    MUTATION_APPROVALS.set("recent-session", {
      timestamp: Date.now() - 1000,
      senderId: "active-user",
    });
    evictStaleApprovals();
    expect(MUTATION_APPROVALS.has("recent-session")).toBe(true);
  });

  it("evicts all stale entries in a single pass", () => {
    const staleTs = Date.now() - APPROVAL_STALENESS_TTL_MS - 1;
    MUTATION_APPROVALS.set("stale-1", { timestamp: staleTs, senderId: "u1" });
    MUTATION_APPROVALS.set("stale-2", { timestamp: staleTs, senderId: "u2" });
    MUTATION_APPROVALS.set("fresh", { timestamp: Date.now(), senderId: "u3" });

    evictStaleApprovals();

    expect(MUTATION_APPROVALS.has("stale-1")).toBe(false);
    expect(MUTATION_APPROVALS.has("stale-2")).toBe(false);
    expect(MUTATION_APPROVALS.has("fresh")).toBe(true);
  });
});

describe("resolveMutationGateConfig", () => {
  /** Helper to build a minimal OpenClawConfig with agent list structure. */
  function cfgWithAgent(
    globalGate: Record<string, unknown> | undefined,
    agentGate: Record<string, unknown> | undefined,
  ): OpenClawConfig {
    return {
      tools: globalGate ? { mutationGate: globalGate } : undefined,
      agents: agentGate ? { list: [{ id: "dev", tools: { mutationGate: agentGate } }] } : undefined,
    } as unknown as OpenClawConfig;
  }

  it("returns undefined when neither global nor agent config exists", () => {
    expect(resolveMutationGateConfig({ cfg: undefined })).toBeUndefined();
    expect(resolveMutationGateConfig({ cfg: {} as unknown as OpenClawConfig })).toBeUndefined();
  });

  it("returns global config when no agent config exists", () => {
    const global = { enabled: true, extraMutations: ["exec"], channels: ["telegram"] };
    const result = resolveMutationGateConfig({
      cfg: cfgWithAgent(global, undefined),
    });
    expect(result).toEqual(global);
  });

  it("returns agent config when no global config exists", () => {
    const agent = { enabled: true, extraMutations: ["write"] };
    const result = resolveMutationGateConfig({
      cfg: cfgWithAgent(undefined, agent),
      agentId: "dev",
    });
    expect(result).toEqual(agent);
  });

  it("merges extraMutations additively from global + agent", () => {
    const result = resolveMutationGateConfig({
      cfg: cfgWithAgent({ enabled: true, extraMutations: ["exec"] }, { extraMutations: ["write"] }),
      agentId: "dev",
    });
    expect(result?.extraMutations).toEqual(["exec", "write"]);
  });

  it("merges channels additively from global + agent (deduplicated)", () => {
    const result = resolveMutationGateConfig({
      cfg: cfgWithAgent(
    ...
    expect(result?.channels?.toSorted()).toEqual(["discord", "telegram"]);
  });

  it("uses global channels when agent has none", () => {
    const result = resolveMutationGateConfig({
      cfg: cfgWithAgent({ enabled: true, channels: ["telegram"] }, { enabled: false }),
      agentId: "dev",
    });
    expect(result?.channels).toEqual(["telegram"]);
  });

  it("uses agent channels when global has none", () => {
    const result = resolveMutationGateConfig({
      cfg: cfgWithAgent({ enabled: true }, { channels: ["slack"] }),
      agentId: "dev",
    });
    expect(result?.channels).toEqual(["slack"]);
  });

  it("agent enabled overrides global enabled", () => {
    const result = resolveMutationGateConfig({
      cfg: cfgWithAgent({ enabled: true }, { enabled: false }),
      agentId: "dev",
    });
...
describe("session-scoped approvals", () => {
  ...
  it("consumes approval when sessionIds match", () => {
    recordMutationApproval(baseArgs.sessionKey, "463420978", "session-A");
    const result = checkMutationGate({
      toolName: "gateway",
      params: {},
      ...baseArgs,
  ...
  it("rejects approval from a different session", () => {
    recordMutationApproval(baseArgs.sessionKey, "463420978", "session-A");
    const result = checkMutationGate({
      toolName: "gateway",
      params: {},
  ...
  it("accepts approval without sessionId from any session (backward compat)", () => {
    recordMutationApproval(baseArgs.sessionKey, "463420978");
    const result = checkMutationGate({
      toolName: "gateway",
      params: {},
  ...
  it("accepts approval with sessionId when consumer has no sessionId", () => {
    recordMutationApproval(baseArgs.sessionKey, "463420978", "session-A");
    const result = checkMutationGate({
      toolName: "gateway",
      params: {},
  ...
  it("stale approval from old session is cleaned up even when rejected", () => {
    recordMutationApproval(baseArgs.sessionKey, "463420978", "session-A");
    // Mismatched session rejects AND deletes the stale entry
    checkMutationGate({
      toolName: "gateway",
      params: {},
      ...baseArgs,
    ...
    // Entry was deleted — even matching session can't find it
    expect(MUTATION_APPROVALS.has(baseArgs.sessionKey)).toBe(false);
  });
});
