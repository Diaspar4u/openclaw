...
describe("a2a-logging handler", () => {
  ...
  describe("postToTelegram", () => {
    beforeEach(() => {
      ...
      mockTelegramFetch.mockImplementation(() =>
        Promise.resolve({
          ok: true,
          status: 200,
          text: () => Promise.resolve("{}"),
    ...
    it("should POST to Telegram API with correct params", async () => {
      ...
      const [url, options] = mockTelegramFetch.mock.calls[0] as unknown as [string, RequestInit];
    ...
    it("should omit message_thread_id when topicId is undefined", async () => {
      ...
      const [, options] = mockTelegramFetch.mock.calls[0] as unknown as [string, RequestInit];
      const body = JSON.parse(options.body as string) as Record<string, unknown>;
      expect(body.message_thread_id).toBeUndefined();
    });

    it("should throw on non-ok response", async () => {
      mockTelegramFetch.mockResolvedValueOnce({
        ok: false,
        status: 403,
  ...
  describe("handler integration", () => {
    ...
    it("should post to Telegram for agent_to_agent:send events", async () => {
      ...
      expect(mockTelegramFetch).toHaveBeenCalledOnce();
      const [url] = mockTelegramFetch.mock.calls[0] as unknown as [string, RequestInit];
